...
class GameSession():
    ...
    def __init__(self, users, session_id) -> None:
        self.users = users
    ...
    def send_chat_message(self, username, msg):
        with self.lock:
            self.channel.basic_publish(exchange=f'session#{self.session_id}', routing_key='',
                                       body=f'{username}: {msg}')
...
class MafiaGame(mafia_pb2_grpc.MafiaServicer):
    ...
    def SendChatMessage(self, request, context):
        username = request.username
        with self.lock:
            self.sessions[request.session_id].send_chat_message(username, request.msg)
        return google.protobuf.empty_pb2.Empty()

    def start_game(self):
        users = list(self.queues.keys())
        session_users = list(filter(lambda x: x not in self.in_game, users))
        self.sessions[self.session_counter] = GameSession(session_users, self.session_counter)
